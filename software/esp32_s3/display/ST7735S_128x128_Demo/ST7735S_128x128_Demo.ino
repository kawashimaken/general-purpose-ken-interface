/**
 * ESP32-S3 + 1.44inch TFT SPI Display Demo
 * Driver  : ST7735S  (128x128, red PCB)
...
public:
  LGFX() {
    // --- SPI bus ---
    {
      ...
      cfg.spi_mode    = 0;
      cfg.freq_write  = 20000000;   // ST7735S: 20 MHz safe limit
    ...
    // --- Panel ---
    {
      ...
      cfg.offset_x         = 2;    // typical 128x128 ST7735S offset
      cfg.offset_y         = 1;
      cfg.offset_rotation  = 0;
      cfg.dummy_read_pixel = 8;
      cfg.dummy_read_bits  = 1;
      cfg.readable         = false;
      cfg.invert           = false; // try true if colors look negative
      cfg.rgb_order        = false; // try true if red and blue are swapped
...
static LGFX        tft;
static LGFX_Sprite canvas(&tft);   // double-buffer sprite (128x128)
...
static const int BALL_COUNT = 8;
static Ball balls[BALL_COUNT];

void initBalls() {
  for (int i = 0; i < BALL_COUNT; i++) {
    balls[i].r     = frand(5, 12);
    balls[i].x     = frand(balls[i].r, 128 - balls[i].r);
    balls[i].y     = frand(balls[i].r, 128 - balls[i].r);
    balls[i].vx    = frand(1.0f, 3.0f) * (random(2) ? 1 : -1);
    balls[i].vy    = frand(1.0f, 3.0f) * (random(2) ? 1 : -1);
...
void demoWave() {
  ...
  for (int y = 0; y < 128; y += 32)
    canvas.drawFastHLine(0, y, 128, (uint16_t)canvas.color888(20, 20, 60));
  for (int x = 0; x < 128; x += 32)
    canvas.drawFastVLine(x, 0, 128, (uint16_t)canvas.color888(20, 20, 60));
  for (int layer = 0; layer < 3; layer++) {
    float   amp  = 28.0f - layer * 7.0f;
    float   freq = 0.05f + layer * 0.02f;
    ...
    int prevY = 64 + (int)(amp * sinf(freq * 0 + waveOffset * (1 + layer * 0.3f)));
    for (int x = 1; x < 128; x++) {
      int curY = 64 + (int)(amp * sinf(freq * x + waveOffset * (1 + layer * 0.3f)));
...
void demoGears() {
  ...
  GearSet gs[] = {
    {64, 64, 6,  1.0f, (uint16_t)canvas.color888(255, 100,   0)},
    {28, 30, 5, -0.8f, (uint16_t)canvas.color888(  0, 200, 255)},
    {98, 95, 4,  0.9f, (uint16_t)canvas.color888(200,  50, 200)},
  };
  for (auto &g : gs) {
    for (int r = 6; r <= 26; r += 5)
...
static const int TRACE_LEN = 200;
...
void demoLissajous() {
  canvas.fillScreen(TFT_BLACK);
  traceX[traceHead] = 64 + (int)(58 * sinf(3.0f * lissPhase + 0.5f));
  traceY[traceHead] = 64 + (int)(58 * sinf(2.0f * lissPhase));
...
void initStars() {
  for (int i = 0; i < STAR_COUNT; i++) {
    stars[i].x = frand(-64, 64);
    stars[i].y = frand(-64, 64);
    stars[i].z = frand(1, 128);
  }
}
...
void demoStarfield() {
  ...
  for (int i = 0; i < STAR_COUNT; i++) {
    Star &s = stars[i];
    s.z -= 2.5f;
    if (s.z <= 0) { s.x = frand(-64,64); s.y = frand(-64,64); s.z = 128; }
    float    sx     = s.x / s.z * 128 + 64;
    float    sy     = s.y / s.z * 128 + 64;
    float    size   = (1.0f - s.z / 128.0f) * 3.5f;
    uint8_t  bright = (uint8_t)((1.0f - s.z / 128.0f) * 255);
...
const char* demoNames[] = {
  "Balls", "Waves", "Gears", "Lissajous", "Stars"
};

void drawHUD() {
  canvas.fillRect(0, 0, 128, 14, (uint16_t)canvas.color888(0, 0, 80));
  canvas.setTextColor(TFT_WHITE);
  canvas.setTextSize(1);
  canvas.setCursor(2, 4);
  canvas.print(demoNames[demoIndex]);
  uint32_t elapsed  = millis() - demoTimer;
  int      progress = (int)(128.0f * elapsed / DEMO_DURATION);
  canvas.drawFastHLine(0, 13, 128,      (uint16_t)canvas.color888( 30,  30,  80));
  canvas.drawFastHLine(0, 13, progress, (uint16_t)canvas.color888(100, 200, 255));
}

// ===== Setup =====
void setup() {
  Serial.begin(115200);
  Serial.println("ESP32-S3 ST7735S 128x128 Demo");
